...
def test_mutation_rate_zero():
    ...
    result = asyncio.run(
        run_simple_experiment(mutation_rate=0.0, iterations=5, population_size=10, checkpoint_dir=None)
    )
...
def test_mutation_rate_one():
    ...
    result = asyncio.run(
        run_simple_experiment(mutation_rate=1.0, iterations=5, population_size=10, checkpoint_dir=None)
    )
    assert result.get_genome_count() > 0
