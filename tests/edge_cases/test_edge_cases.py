...
def test_population_size_one():
    ...
    result = run_simple_experiment(population_size=1, iterations=5, mutation_rate=0.5)
...
def test_zero_iterations():
    ...
    population_size = 10
    result = run_simple_experiment(iterations=0, population_size=population_size)
...
def test_mutation_rate_zero():
    ...
    result = run_simple_experiment(mutation_rate=0.0, iterations=5, population_size=10)
...
def test_mutation_rate_one():
    ...
    result = run_simple_experiment(mutation_rate=1.0, iterations=5, population_size=10)
    assert result.get_genome_count() > 0
